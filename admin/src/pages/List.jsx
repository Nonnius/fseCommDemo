...
const List = ({token}) => {
   //data retreiver from api
  const [list, setList] = useState([])

  const fetchList = async () =>{  //this function will run whenever page reload
    try {
       //remember to refer to productRoute.js from backend
      const response = await axios.get(backendUrl + '/api/product/listProducts')
      //console.log(response.data) only for testing       
          ...
          list.map((item, index)=>(
            ...
            key={index}>
              ...
              className='text-right md:text-center cursor-pointer text-lg'>
                <MdDeleteForever className='ml-[4rem]'/>
              </p>
            </div>
          ))
        }
