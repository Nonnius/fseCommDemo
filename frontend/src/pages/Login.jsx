...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (currentState === 'Sign Up') {
        ...
        //console.log(response.data); only to test trigger
  ...
  return (
    ...
    text-gray-800'>
      ...
      type='text' className='w-full px-3 border border-gray-800'
      placeholder='Name' required/>}
      <input onChange={(e)=>setEmail(e.target.value)} value={email} 
      type='email' className='w-full px-3 border border-gray-800'
      placeholder='Your email' required/>
      <input onChange={(e)=>setPassword(e.target.value)} value={password}  
      type='password' className='w-full px-3 border border-gray-800'
      placeholder='password' required/>
      <div className='w-full flex justify-between text-sm mt-[-14px]'>
      ...
      <button className='bg-black text-white font-light px-8 py-2 mt-4 mb-5'>
